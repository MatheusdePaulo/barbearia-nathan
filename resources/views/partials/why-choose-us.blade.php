        ...
        <div class="flex flex-col md:grid md:grid-cols-3 items-center gap-12 md:gap-8 lg:gap-16 w-full">
            ...
            <div class="flex flex-row md:contents items-center justify-between w-full gap-4 md:gap-0">
                ...
                <div class="flex flex-col items-center text-center flex-1 md:order-3 md:w-auto">
                    ...
                    <p class="font-barlow font-bold text-[10px] md:text-xs text-[#1A1C1E] uppercase tracking-widest mb-1">Nathan do Corte</p>
                    @if(isset($reviewAverage) && $reviewAverage)
                        <span class="font-barlow font-black text-4xl md:text-6xl text-[#1A1C1E] leading-none mb-2">{{ $reviewAverage }}</span>
                        @php
                            $stars = round((float)$reviewAverage);
                        @endphp
                        <div class="flex text-[#DEC7A6] text-[8px] md:text-[10px] mb-1">
                            @for($i = 1; $i <= 5; $i++)
                        ...
                        <p class="font-work-sans text-[10px] md:text-[11px] font-medium text-zinc-400 uppercase tracking-widest">{{ $reviewTotal ?? 0 }} {{ ($reviewTotal ?? 0) === 1 ? 'avaliação' : 'avaliações' }}</p>
                    @else
                        <span class="font-barlow font-black text-4xl md:text-6xl text-[#1A1C1E] leading-none mb-2">—</span>
                        <div class="flex text-[#DEC7A6]/30 text-[8px] md:text-[10px] mb-1">
                            <i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i>
                        </div>
